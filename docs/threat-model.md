...
3. **open·kritt ↔ model/provider.** Repository content is sent to the configured
   OpenAI/Codex, Anthropic, OpenRouter, xAI, or Abliteration endpoint. That's a data-egress boundary.
...
- Know **where your data goes** before scanning sensitive code. The supported setup paths
  use Codex/OpenAI, Anthropic, OpenRouter, xAI (Grok Build), or Abliteration credentials.
