...
const MANAGED_CREDENTIAL_PROVIDERS = new Set(['openrouter', 'abliteration']);
...
export function validateProviderCredential(provider, credential) {
  if (!MANAGED_CREDENTIAL_PROVIDERS.has(provider)) {
    return { field: 'provider', message: 'Only OpenRouter and Abliteration use an API key in Accounts.' };
  }
