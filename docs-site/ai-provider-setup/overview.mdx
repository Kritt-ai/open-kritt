...
| **xAI (Grok)** | `XAI_API_KEY` (direct api.x.ai) | Curated Grok list (`grok-4.5` default) |
...
<CardGroup cols={2}>
  ...
  <Card title="xAI (Grok)" icon="bolt" href="/ai-provider-setup/xai">
    Direct Grok 4.5 via the xAI API — not OpenRouter.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Choose one access method">
    ...
    **OpenRouter API key**, or **xAI API key**. Follow the provider guide above to obtain
    ...
    login is available from the running application's **Accounts** page.
  </Step>
