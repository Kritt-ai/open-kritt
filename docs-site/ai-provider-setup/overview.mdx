...
| **Abliteration** | `ABLIT_KEY` | Authenticated catalog picker |
...
<CardGroup cols={2}>
  ...
  <Card title="Abliteration" icon="rocket" href="/ai-provider-setup/abliteration">
    Run less-restrictive Abliteration.ai models on the Codex harness.
  </Card>
</CardGroup>
...
<Steps>
  ...
  <Step title="Choose one access method">
    ...
    **OpenRouter API key**, **xAI API key**, or **Abliteration API key**. Follow the
    provider guide above to obtain the correct credential. The CLI configures keys and
    Codex login; Claude and xAI device login, plus OpenRouter/xAI/Abliteration key
    management, are also available from the running application's **Accounts** page.
  </Step>
...
its OpenRouter, xAI, and Abliteration keys synchronized with `.env`, and mirrors those keys under
...
[Abliteration](/ai-provider-setup/abliteration).
