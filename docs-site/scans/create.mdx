...
| `model_provider` | Yes | `codex`, `claude`, `openrouter`, `xai`, or `abliteration`. |
...
`openrouter` + `claude-code` or advanced `codex`, `xai` + `grok-build`, and
`abliteration` + `codex`. The API returns `422`
